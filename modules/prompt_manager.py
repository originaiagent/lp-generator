...
【最重要ルール - テキストの扱い】
- 提供されたテキスト（見出し、本文、キャッチコピー等）は一字一句変更せず、そのまま使用してください
- 文言の省略、言い換え、要約は絶対に禁止です
- 「〜しましょう」を「〜する」に変えるなどの微細な変更も禁止です
- テキストが長い場合でも、省略せずにレイアウトで工夫してください
...
{{
    ...
    "reason": "この修正により〇〇が改善されます"
        ...
        "employee_evaluation": {
            "name": "メンバーAI評価",
            "description": "特定の役割を持つメンバーの視点でLPを評価（過去のフィードバックを学習可能）",
            "template": """あなたは、以下のプロフィールを持つ当社のメンバーとして、このLPを専門的な視点で評価してください。
...
（具体的なアドバイス）"""
        ...
        "employee_evaluation_revision": {
            "name": "メンバーAI再評価",
            "description": "フィードバックを反映してメンバーAIの評価を再生成する",
            "template": """あなたは、以下のプロフィールを持つ当社のメンバーとして、このLPを専門的な視点で評価してください。
...
（具体的なアドバイス）"""
        ...
        "employee_profile_update": {
            "name": "メンバープロフィール更新",
            "description": "フィードバックを基にメンバーAIのプロフィールを更新する提案を生成",
            "template": """あなたはメンバーAIプロフィールの管理者です。
以下のメンバーが行ったLP評価に対して、担当者からフィードバックがありました。
このフィードバックを分析し、メンバープロフィールのどのフィールドをどう更新すべきか提案してください。
  ...
  "candidates": [
    ...
    {{"direction": "方向性の説明", "text": "提案コピー"}}
        ...
        "wireframe_generation": {
            ...
            "template": """【最重要・厳守】出力画像のアスペクト比は、入力画像と1ピクセルも違わず完全に同一にすること。
入力画像が横長なら横長、正方形なら正方形で出力。絶対に縦横比を変更しないこと。
このアスペクト比の維持は、他のどの指示よりも優先される最重要事項です。

あなたは、熟練したWebディレクター兼UI/UXデザイナーです。
アップロードされた「完成されたWebデザインの画像」を分析し、そのデザインの骨子となる「構成案（ワイヤーフレーム）画像」を生成してください。

## 画像生成のルール（Output Style）

1. **トーン＆マナー:**
* 完全なモノクロ（グレースケール）で出力する。背景は薄いグレー（#F2F2F2程度）、要素は白または濃いグレーのボックスで表現する。
* 手書き風ではなく、デジタルツール（FigmaやAdobe XD）で作ったような直線的でクリーンなラインを使用する。

2. **画像要素の処理（Placeholders）:**
* 元の画像にある「写真」や「イラスト」はすべて抽象化する。
* 長方形のボックスの中に「×（バッテン）」を描き、中央に「MAIN IMAGE」「IMAGE」などのラベルを配置する。
* 具体的な人物や商品は描画しない。

3. **テキスト要素の処理:**
* 見出し（キャッチコピー）など、構成上重要なテキストは可能な限り元の意味や配置を維持して再現する。
* 詳細な本文は「横線（―）」や「ダミーテキスト」のブロックとして表現し、文字量と段落の構造だけを示す。

4. **注釈の排除:**
* 生成する画像には、「Element 1」「Element 2」のような構成要素を示す注釈やラベルは一切含めないこと。
* 純粋にデザインのレイアウトとコンテンツ構造のみを示す、クリーンなワイヤーフレームとする。

## プロセス
1. アップロードされた画像を上から下までスキャンし、セクションと要素の配置を特定する。
2. 各セクションの画像とテキストの比率、配置パターンを把握する。
3. 分析結果に基づき、上記のルールに従って、注釈のない新しいワイヤーフレーム画像を生成する。"""
        }
    }
